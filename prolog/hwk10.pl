...
schedule_data([schedule(c1, r3, between(9, 10)),
...
legal_schedule(X) :-
	schedule(C, R, between(Start,End))=X,
	course(C, Ccount, Cmedia, Ctime),
	room(R, Rcount, Rmedia),
	(End-Start)=:=Ctime,
	Ccount=<Rcount,
	media_compatible(Cmedia,Rmedia).
