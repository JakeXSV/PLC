/* Correct HW11 - Written by Zach Hart */
...
legal_schedule(schedule(C,R,between(S,E))):-
	course(C,Enrolled,CMed,Dur),
	room(R,Cap,RMed),
	Enrolled=<Cap,
	media_compatible(CMed,RMed),
...
check_conflict(_, []).
check_conflict(S, [S1 | L]) :- 
	no_conflict(S, S1), 
	check_conflict(S, L).

legal_schedule_list([]).
legal_schedule_list([S | L]) :- 
	legal_schedule_list(L),
	legal_schedule(S),
	check_conflict(S, L).
